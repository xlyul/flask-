...
$(document).ready(function () {
    ...
    $(".filter-item-bar>.filter-area").on("click", "li", function (e) {
        if (!$(this).hasClass("active")) {
            ...
            location.href ='/house/search/' + location.search.replace('aid=' + aid, 'aid=' + new_id)
...
$(document).ready(function () {
    ...
    $.get('/house/allsearch/' + path, function (data) {
        if (data.code == '200') {
            var house_info = template('house-info', {ohouses: data.hlist});
            $('.house-list').html(house_info);

            var area = template('area', {areas: data.area_list});
            $('.filter-area').append(area)
        }
    })
});